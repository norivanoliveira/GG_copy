...
function Buff:CreateBuffs(unit)
    ...
    for i = 0, buffCount do
        ...
        if buff then
        	...
        	if name and count and #name < 1000 and count > 0 then
	            result[name:lower()] =
	            {
	                Type = buff.type,
...
function SummonerSpell:UseCleanse(s1, s2)
    ...
    for i, hero in ipairs(Object:GetCachedHeroes()) do
...
function Object:ResetCachedObjects()
	self.CachedHeroes = {}
	self.CachedMinions = {}
	self.CachedTurrets = {}
	self.CachedWards = {}
...
function Object:GetCachedHeroes()
	if #self.CachedHeroes == 0 then
	    ...
	    for i = 1, count do
	        ...
	        if obj and obj.valid and obj.visible and obj.isTargetable and obj.dead == false then
	        	table_insert(self.CachedHeroes, obj)
        	end
	    end
	end
	return self.CachedHeroes
...
function Object:GetEnemyHeroes(range, bbox, visible, immortal, jaxE, func)
    local result = {}
    for i, obj in ipairs(self:GetCachedHeroes()) do
        if self:IsHeroValid(obj, immortal, jaxE) and obj.isEnemy then
...
function Object:GetCachedMinions()
	if #self.CachedMinions == 0 then
		local count = Game.MinionCount()
		if count == nil or count < 0 or count > 10000 then
			print('game.minioncount=' .. tostring(count))
			return self.CachedMinions
		end
	    for i = 1, count do
	        local obj = Game.Minion(i)
	        if obj and obj.valid and obj.visible and obj.isTargetable and obj.dead == false and obj.isImmortal == false then
...
function Object:GetEnemyMinions(range, bbox, visible, immortal, func)
    local result = {}
    for i, obj in ipairs(self:GetCachedMinions()) do
...
function Object:GetCachedWards()
	if #self.CachedWards == 0 then
		local count = Game.WardCount()
		if count == nil or count < 0 or count > 10000 then
			print('game.wardcount=' .. tostring(count))
			return self.CachedWards
		end
	    for i = 1, count do
...
function Object:GetOtherAllyMinions(range, bbox, visible, immortal, func)
    local result = {}
    for i, obj in ipairs(self:GetCachedWards()) do
        if obj.isAlly then
            if (not range or obj.distance < range + (bbox and obj.boundingRadius or 0)) and (not func or func(obj)) then
                table_insert(result, obj)
            end
...
function Object:GetCachedTurrets()
	if #self.CachedTurrets == 0 then
		local count = Game.TurretCount()
		if count == nil or count < 0 or count > 10000 then
			print('game.turretcount=' .. tostring(count))
			return self.CachedTurrets
		end
	    for i = 1, count do
	        local obj = Game.Turret(i)
	        if obj and obj.valid and obj.visible and obj.isTargetable and obj.dead == false and obj.isImmortal == false then
	        	table_insert(self.CachedTurrets, obj)
        	end
	    end
	end
	return self.CachedTurrets
...
function Object:GetEnemyTurrets(range, bbox, visible, immortal, func)
    local result = {}
    for i, obj in ipairs(self:GetCachedTurrets()) do
        if obj.isEnemy then
            if (not range or obj.distance < range + (bbox and obj.boundingRadius or 0)) and (not func or func(obj)) then
                table_insert(result, obj)
            end
...
function Health:OnTick()
    ...
    for i, obj in ipairs(Object:GetCachedMinions()) do
        if Math:IsInRange(myHero, obj, 2000) then
            table_insert(self.CachedMinions, obj)
        end
    end
    for i, obj in ipairs(Object:GetCachedWards()) do
        if obj.isEnemy and Math:IsInRange(myHero, obj, 2000) then
            table_insert(self.CachedWards, obj)
        end
    end

    for i, obj in ipairs(self.CachedMinions) do
    ...
    for i, obj in ipairs(self.CachedWards) do
    ...
    for i, obj in ipairs(structures) do
