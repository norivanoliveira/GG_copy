...
do
    ...
    function Cached:GetMinions()
        if not self.MinionsSaved then
            ...
            if count and count > 0 and count < 500 then
                local index = 0
                for i = 1, count do
...
Callback.Add('Load', function()
    local loads = SDK.Load
    local draws = SDK.Draw
    local ticks = SDK.Tick
    local wndmsgs = SDK.WndMsg
    local fastticks = SDK.FastTick
    local draw = Menu.Main.Drawings.Enabled
    
    for i = 1, #loads do loads[i]() end

    Callback.Add("Draw", function()
        Cached:Reset()
        for i = 1, #fastticks do fastticks[i]() end
        if not draw:Value() then return end
        for i = 1, #draws do draws[i]() end
    end)
    ...
    Callback.Add("WndMsg", function(msg, wParam)
        for i = 1, #wndmsgs do wndmsgs[i](msg, wParam) end
    end)
